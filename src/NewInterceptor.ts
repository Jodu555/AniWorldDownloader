	...
	async intercept(url: string): Promise<string> {
		return new Promise(async (resolve, reject) => {
			...
			this.interval = setInterval(async () => {
				if (isNavigating) {
					console.log('Page is navigating, skipping tick...');
					return;
				}
				...
				try {
					...
					if (
						...
						console.log('Context destroyed during navigation, waiting for page to settle...');
						try {
							...
							// waitForNavigation may throw if already settled — ignore
						}
						return;
					}
					console.error('Unexpected error in intercept interval', err);
					return;
				}
				...
				if (m3u8 == 'VOE') {
					try {
						...
						if (err?.message?.includes('Execution context was destroyed')) return;
						console.error('VOE frame eval error', err);
					}
					...
					try {
						...
						if (err?.message?.includes('Execution context was destroyed')) return;
						console.error('Vidoza frame eval error', err);
					}
					...
					try {
						...
						if (err?.message?.includes('Execution context was destroyed')) return;
						console.error('Streamtape frame eval error', err);
					}
				}
